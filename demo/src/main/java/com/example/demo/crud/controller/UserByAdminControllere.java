...
public class UserByAdminControllere {
    ...
    private final ConcertOwnerRepository concertOwnerRepository;
...
public String getPurchaseHistory(@PathVariable("UserId") Integer userId, Model model){
    ...
    // }
     ...
     @GetMapping("/concert_owner")
    public String getConcertOwner(Model model){
        List<ConcertOwner> concertOwners = concertService.getConcertOwnerByAdmins();
        model.addAttribute("concertOwners", concertOwners);
        return "concertowner";
    }

    @GetMapping("/search_concert_owner")
public String searcgConcertOwner(@RequestParam(name = "concertName", required = false) String concertName, Model model) {
    List<ConcertOwner> concertOwners;
    if (concertName == null || concertName.isEmpty()) {
        concertOwners = concertOwnerRepository.findAll();
    } else {
        concertOwners = concertOwnerRepository.findByConcertNameContainingIgnoreCase(concertName);
    }
    model.addAttribute("concertOwners", concertOwners);
    model.addAttribute("concertName", concertName);
    return "concertowner";
}
