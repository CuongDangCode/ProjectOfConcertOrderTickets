...
public String getPurchaseHistory(@PathVariable("UserId") Integer userId, Model model){
    ...
    public String getConcertOwner(Model model){
        List<ConcertOwner> concertOwners = cinemaService.getConcertOwnerByAdmins();
...
public String searchTickByConcertName(@RequestParam(name = "moiveName", required = false) String moiveName, Model model) {
    List<Ticket> tickets;
    if (moiveName == null || moiveName.isEmpty()) {
        tickets = ticketService.getAllTickets();
    } else {
        tickets = ticketRepository.findTicketsByConcertTitleContainingIgnoreCase(moiveName);
    }
    model.addAttribute("tickets", tickets);
