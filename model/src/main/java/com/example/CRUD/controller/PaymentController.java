...
import com.example.CRUD.service.ConcertOwnerTransactionService;
...
public class PaymentController {
    ...
    @Autowired
    private ConcertOwnerTransactionService concertOwnerTransactionService;
...
public String paymentCompleted(HttpServletRequest request, Model model, Principal principal, HttpSession session) {
    ...
    public void processRefundAndTransfer(Ticket ticket) {
        ...
        Integer concertOwnerId = yardService.findConcertOwnerIdByYardId(ticket.getYard().getYardID());
        // ConcertOwner concertOwner = cinemaService.getConcertOwnerById(concertOwnerId);
        ...
        // Transfer 25% to concert owner's wallet
        // concertOwner.addFundsToWallet(transferAmount);
        // cinemaService.saveConcertOwner(concertOwner);
        ...
        // Add transfer transaction for concert owner
        ConcertOwnerTransaction concertOwnerTransaction = new ConcertOwnerTransaction();
        // concertOwnerTransaction.setConcertOwner(concertOwner);
        concertOwnerTransaction.setTransactionType("Transfer");
        concertOwnerTransaction.setAmount(transferAmount);
        concertOwnerTransaction.setTransactionDate(new Date());
        concertOwnerTransactionService.saveTransaction(concertOwnerTransaction);
    ...
    // Transfer remaining funds from intermediary wallet to concert owner
    // @PostMapping("/admin/transferToConcertOwner")
    // public String transferToConcertOwner(@RequestParam("concertOwnerId") int concertOwnerId, Model model) {
    //     ConcertOwner concertOwner = cinemaService.getConcertOwnerById(concertOwnerId);

    //     if (concertOwner == null) {
    //         logger.error("Concert Owner not found with ID: " + concertOwnerId);
    //         model.addAttribute("error", "Concert Owner not found.");
