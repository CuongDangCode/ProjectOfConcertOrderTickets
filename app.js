...
let data = pvr.filter(obj => obj.date == main_date && obj.concert  == url_segment[1]);
// console.log(data);

document.getElementById('title').innerText = data[0].concert;
...
document.getElementById('book_ticket').addEventListener('click', () => {
    Array.from(document.getElementsByClassName('selected')).forEach(el => {
        ...
        let obj = {
            concert: url_segment[1],
            date : main_date
        }

        let getData = pvr.map((obj) => {
            if (
                obj.concert === url_segment[1] && obj.date === main_date
            ) {
        ...
        document.getElementById('chair').innerHTML = '';
        let data = getData.filter(obj => obj.date === main_date && obj.concert === url_segment[1]);
        addSeats(data);
        ...
        tic.innerHTML = `
                    ...
                    <div class="tic_details" style=" background: url(${data[0].background})no-repeat center -35px /cover">
                        ...
                        <h5 class="pvr"><span>Vegus</span> Concert</h5>
                        <h1>${url_segment[1]}</h1>
                        <div class="seat_det">
                            <div class="seat_cr">
                                <h6>ROW</h6>
